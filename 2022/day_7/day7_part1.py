...
class Dir:
    ...
    def populate_list(self, size_list, max_size):
        for item in self.contents:
            if type(item) is Dir:
                item.populate_list(size_list, max_size)
        if self.get_dir_size() <= max_size:
...
def traverse_tree(file_tree):
    size_list = []
    file_tree.populate_list(size_list, 100000)
    return size_list


if __name__ == '__main__':
    print(sum(traverse_tree(build_file_tree())))
